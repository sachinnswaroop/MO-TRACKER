  ...
  return (
    <button
      ...
      className={`group rounded-2xl border p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md ${
        solid ? `bg-gradient-to-br ${def.gradient} border-transparent text-white` : "border-ink-200 bg-white"
      }`}
    >
      ...
      <div
        className={`mt-3.5 grid grid-cols-3 gap-2 rounded-xl p-2.5 ${
          solid ? "bg-white/15" : "bg-ink-50"
        }`}
      >
        {[
          ...
          <div key={label as string} className="min-w-0">
            ...
            <div className={`text-lg font-extrabold ${solid ? "text-white" : "text-ink-900"}`}>{num}</div>
